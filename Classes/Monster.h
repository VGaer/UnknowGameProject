...
public:
	...
	TimeCounter* m_remoteskill_projectiledur_timecounter;//怪物远程投掷（持续）技能生成计时器
	TimeCounter* m_remoteskilldur_timecounter;//怪物远程（持续）技能cd计时器
	bool checkInBigSkillRange();	//是否在大技能的攻击范围
	bool checkInRemoteSkillRange(); //是否在远程技能的攻击范围
	bool checkInRemoteSkillDurRange(); //是否在远程技能（持续）的攻击范围
	// -1代表没有技能
