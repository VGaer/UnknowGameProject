...
bool Monster::init(const std::string& name)
{
	...
	m_remoteskilldur_timecounter = TimeCounter::create();
	this->addChild(m_remoteskilldur_timecounter);
	m_remoteskilldur_timecounter->start();
	...
	m_remoteskill_projectiledur_timecounter = TimeCounter::create();
	this->addChild(m_remoteskill_projectiledur_timecounter);
...
bool Monster::checkInRemoteSkillDurRange()
{
	if (player != NULL)
	{
		//如果远程技能存在
		if (monsdata.remoteskillmap.find("remoteskilldur") != monsdata.remoteskillmap.end())
		{
			switch (animBase->getCurDirection())
			{
			case Dir_up:{
