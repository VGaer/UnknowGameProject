...
void AnimBase::playAttaAnim()
{
	...
	else if (direction >= 0 && direction < 3 && m_monster->m_curskill == "remoteskilldur")
	{
		m_sprite->runAction(m_attaRemoteDurAnimArray[direction]);
	}
...
//	Animation* animation = AnimationUtil::createWithSingleFrameName("treemonsterhstatic", 0.1f, 1);
	...
	//远程技能(持续)测试
	if (m_monster->monsdata.remoteskillmap.find("remoteskilldur") != m_monster->monsdata.remoteskillmap.end())
	{
		animation = AnimationUtil::createWithSingleFrameName(m_monster->monsdata.remoteskillmap["remoteskilldur"].hAnimateName.c_str(), m_monster->monsdata.remoteskillmap["remoteskilldur"].attackAnimatetimePer, 1);
		animate = Animate::create(animation);
		animate->retain();
		m_attaRemoteDurAnimArray[0] = animate;

		animation = AnimationUtil::createWithSingleFrameName(m_monster->monsdata.remoteskillmap["remoteskilldur"].uAnimateName.c_str(), m_monster->monsdata.remoteskillmap["remoteskilldur"].attackAnimatetimePer, 1);
		animate = Animate::create(animation);
		animate->retain();
		m_attaRemoteDurAnimArray[1] = animate;

		animation = AnimationUtil::createWithSingleFrameName(m_monster->monsdata.remoteskillmap["remoteskilldur"].dAnimateName.c_str(), m_monster->monsdata.remoteskillmap["remoteskilldur"].attackAnimatetimePer, 1);
		animate = Animate::create(animation);
		animate->retain();
		m_attaRemoteDurAnimArray[2] = animate;
