...
void PopLayer::onEnter() {
	...
	if (getLabelContentText()) {
		...
		ltf->setPosition(Vec2(55, contentSize.height - 35 - getLabelTitle()->getContentSize().height - 10));
...
{
	string str = temp.substr(0, this->n);
	getLabelContentText()->setString(str);
	n = n + 3;
	if (n > temp.length()) {
		this->unschedule(schedule_selector(PopLayer::showFont));
		temp = "";
		n = 3;
	}
}
