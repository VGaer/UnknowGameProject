...
void Attack::Enter(Monster* target)
{
	...
	else if (target->m_curskill == "remoteskilldur")
	{
		//远程技能的冷却计时
		target->m_remoteskilldur_timecounter->start();
		//设置怪物朝向
		target->getAnimBase()->setCurDirection(target->getPlayer()->getPosition());
	}
...
void Attack::Excute(Monster* target)
{
	...
	if (target->m_curskill == "baseskill"){
		...
		if (target->m_baseskill_timecounter->getCurTime() > target->monsdata.skillmap["baseskill"].attackEnd)
		{
			...
			//如果远程技能（持续）存在
			if (target->monsdata.remoteskillmap.find("remoteskilldur") != target->monsdata.remoteskillmap.end())
			{
				/*如果在远程技能（持续）攻击范围下，且远程技能（持续）冷却时间完成,变成远程（持续）攻击*/
				if (target->checkInRemoteSkillDurRange() && target->m_remoteskilldur_timecounter->getCurTime() > target->monsdata.remoteskillmap["remoteskilldur"].attackInter)
				{
					target->m_curskill = "remoteskilldur";
					target->getStateMachine()->ChangeState(new Attack());
					return;
				}
			}
			...
			{
				/*如果在远程技能攻击范围下，且远程技能攻击冷却完,变成远程攻击*/
	...
	//如果当前技能时远程技能（持续）
	if (target->m_curskill == "remoteskilldur"){
		/*远程技能(持续)动作动画播放在后摇，才投掷物品*/
		if (target->m_remoteskilldur_timecounter->getCurTime() > target->monsdata.remoteskillmap["remoteskilldur"].beforeattacktimes
			&& target->m_remoteskilldur_timecounter->getCurTime() < target->monsdata.remoteskillmap["remoteskilldur"].attackEnd)
		{
			//远程技能（持续）投掷物生成cd完了
			if (target->m_remoteskill_projectiledur_timecounter->getCurTime() == 0.0f ||
				target->m_remoteskill_projectiledur_timecounter->getCurTime() > target->monsdata.remoteskillmap["remoteskilldur"].attackInter)
			{
				target->m_remoteskill_projectiledur_timecounter->start();

				//远程技能new一个投掷物
				auto remoteskillstr = target->monsdata.remoteskillmap["remoteskilldur"];
				std::string projectileName = remoteskillstr.projectileName;
				std::string projectileAniamteName = remoteskillstr.projectileAnimateName;
				float width = remoteskillstr.skillwidth;
				float height = remoteskillstr.skillheight;
		...
		//远程技能(持续)动画播放完了
		if (target->m_remoteskilldur_timecounter->getCurTime() > target->monsdata.remoteskillmap["remoteskilldur"].attackEnd)
		{
			//如果大技能存在
			if (target->monsdata.skillmap.find("bigskill") != target->monsdata.skillmap.end())
			{
			...
			if (target->m_remoteskilldur_timecounter->getCurTime() < target->monsdata.remoteskillmap["remoteskilldur"].beforeattacktimes)
			{
				...
				{
					//可以中断，进入被击状态//如果被打断了，投掷物就发射不出去的
					if (target->monsdata.remoteskillmap["remoteskilldur"].Isinter == true)
					{
						target->getStateMachine()->ChangeState(new Attacked());
						return;
					}
